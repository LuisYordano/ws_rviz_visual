...
class RvizMarkers(Node):
    ...
    def __init__(self, base_frame, marker_topic, wait_time=None):
        super().__init__('RvizMarkersNode_')
    ...
    def setDefaultMarkerParams(self):
        ...
        self.marker_lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        self.muted = False
        self.alpha = 1.0
    ...
    def waitForSubscriber(self, publisher, wait_time=1.0):
        ...
        while (num_existing_subscribers == 0):
            ...
            if (self.get_clock().now().to_msg() > max_time):
                self.get_logger().error("No subscribers connected to the '%s' topic after %f seconds", self.marker_topic, wait_time)
                return False
    ...
    def getColor(self, color):
        ...
        if (type(color) == tuple) or (type(color) == list):
            ...
            self.get_logger().error("getColor() called with unknown color name '%s', defaulting to 'blue'", color)
            result.r = 0.1
            result.g = 0.1
            result.b = 0.8
    ...
    def publishSphere(self, pose, color, scale, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishSphere()", type(pose).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishSphere()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            sphere_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            sphere_marker.lifetime = Duration(seconds=lifetime).to_msg()  # in seconds
    ...
    def publishArrow(self, pose, color, scale, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishArrow()", type(pose).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishArrow()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            arrow_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            arrow_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishCube(self, pose, color, scale, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishCube()", type(pose).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishCube()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            cube_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            cube_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishCubes(self, list_of_cubes, color, scale, lifetime=None):
        ...
        if type(list_of_cubes) != list:
            self.get_logger().error("list_of_cubes is unsupported type '%s' in publishCubes()", type(list_of_cubes).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishCubes()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            cubes_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            cubes_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
        ...
        for i in range(0, len(list_of_cubes)):
            ...
            if type(list_of_cubes[i]) == Pose:
                ...
                self.get_logger().error("list_of_cubes contains unsupported type '%s' in publishCubes()", type(list_of_cubes[i]).__name__)
                return False
    ...
    def publishCylinder(self, pose, color, height, radius, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishCylinder()", type(pose).__name__)
            return False
        ...
        if lifetime == None:
            cylinder_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            cylinder_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishAxis(self, pose, length, radius, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishAxis()", type(pose).__name__)
            return False

        
        t = tf_transformations.translation_matrix( (length/2.0, 0.0, 0.0) )
        r = tf_transformations.rotation_matrix(numpy.pi/2.0, (0,1,0))
        m = tf_transformations.concatenate_matrices(axis_pose, t, r)
        x_pose = mat_to_pose(m)
        self.publishCylinder(x_pose, 'red', length, radius, lifetime)

        t = tf_transformations.translation_matrix( (0.0, length/2.0, 0.0) )
        r = tf_transformations.rotation_matrix(numpy.pi/2.0, (1,0,0))
        m = tf_transformations.concatenate_matrices(axis_pose, t, r)
        y_pose = mat_to_pose(m)
        self.publishCylinder(y_pose, 'green', length, radius, lifetime)

        t = tf_transformations.translation_matrix( (0.0, 0.0, length/2.0) )
        r = tf_transformations.rotation_matrix(0.0, (0,0,1))
        m = tf_transformations.concatenate_matrices(axis_pose, t, r)
    ...
    def publishMesh(self, pose, file_name, color, scale, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishMesh()", type(pose).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishMesh()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            mesh_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            mesh_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishRectangle(self, point1, point2, color, lifetime=None):
        ...
        if type(point1) == Point:
            ...
            self.get_logger().error("Point1 is unsupported type '%s' in publishRectangle()", type(point1).__name__)
            return False
        if type(point2) == Point:
            rect_point2 = point2
        else:
            self.get_logger().error("Point2 is unsupported type '%s' in publishRectangle()", type(point2).__name__)
            return False
        ...
        if lifetime == None:
            rectangle_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            rectangle_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishPlane(self, pose, depth, width, color, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishRectangle()", type(pose).__name__)
            return False
    ...
    def publishLine(self, point1, point2, color, width, lifetime=None):
        ...
        if type(point1) == Point:
            ...
            self.get_logger().error("Point1 is unsupported type '%s' in publishLine()", type(point1).__name__)
            return False
        ...
        if type(point2) == Point:
            ...
            self.get_logger().error("Point2 is unsupported type '%s' in publishLine()", type(point2).__name__)
            return False
        ...
        if lifetime == None:
            line_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            line_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
    ...
    def publishPath(self, path, color, width, lifetime=None):
        ...
        if type(path) == list:
            ...
            self.get_logger().error("Path is unsupported type '%s' in publishPath()", type(path).__name__)
            return False
        ...
        if lifetime == None:
            path_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            path_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
        ...
        for i in range(1, len(path)):
            ...
            if type(path[i]) == Point:
                ...
                self.get_logger().error("path list contains unsupported type '%s' in publishPath()", type(path[i]).__name__)
                return False
    ...
    def publishPolygon(self, polygon, color, width, lifetime=None):
        ...
        if type(polygon) == Polygon:
            ...
            self.get_logger().error("Path is unsupported type '%s' in publishPolygon()", type(polygon).__name__)
            return False
    ...
    def publishSpheres(self, list_of_spheres, color, scale, lifetime=None):
        ...
        if type(list_of_spheres) != list:
            self.get_logger().error("list_of_spheres is unsupported type '%s' in publishSpheres()", type(list_of_spheres).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishSpheres()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            spheres_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            spheres_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
        ...
        for i in range(0, len(list_of_spheres)):
            ...
            if type(list_of_spheres[i]) == Pose:
                ...
                self.get_logger().error("list_of_sphere contains unsupported type '%s' in publishSphere()", type(list_of_spheres[i]).__name__)
                return False
    ...
    def publishText(self, pose, text, color, scale, lifetime=None):
        ...
        if (type(pose) == numpy.matrix) or (type(pose) == numpy.ndarray):
            ...
            self.get_logger().error("Pose is unsupported type '%s' in publishText()", type(pose).__name__)
            return False
        ...
        if type(scale) == Vector3:
            ...
            self.get_logger().error("Scale is unsupported type '%s' in publishText()", type(scale).__name__)
            return False
        ...
        if lifetime == None:
            text_marker.lifetime = Duration(seconds=0.0).to_msg() # 0 = Marker never expires
        else:
            text_marker.lifetime = Duration(seconds=lifetime).to_msg() # in seconds
...
def pose_to_mat(pose):
    ...
    mat = numpy.matrix(tf_transformations.quaternion_matrix(quat))
    mat[0:3, 3] = pos
